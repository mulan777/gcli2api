...
def _normalize_antigravity_request(
    ...
    # 1. 思考模型处理：官方注释写 antigravity 模型名不带 -high/-low/-search，
    # 但线上原生 ID（如 gemini-3.1-pro-high）把档位写在模型名里，必须原样保留。
    # 其余模型仅通过 "think" 是否出现在模型名中判断，命中则使用默认思考预算。
    ...
    if original_model in ANTIGRAVITY_NATIVE_MODEL_IDS:
